...
让 agent 通过每日反思，将经验写回行为规则文件，形成学习闭环。

### 架构原则

- **数据源**：`memory/YYYY-MM-DD.md`（当日对话原始日志）
- **调度**：HEARTBEAT.md + 时间窗口判断（避免每次心跳都触发）
- **幂等性**：在 memory 文件末尾写 `<!-- REVIEWED: -->` 标记，防止重复执行
- **写回分级**：

| 类型 | 写入目标 | 是否需要用户确认 |
|------|---------|-----------------|
| 流程性规则 | AGENTS.md | 否，直接写入 |
| 操作级约束 | SOUL.md | 是，SOUL.md 为高权限文件 |
| 经验积累 | LESSONS.md | 否，追加写入 |

- **会话读取**：AGENTS.md 中配置每次会话开始时读取 LESSONS.md，让经验真正影响行为

### 关键约束

- SOUL.md 不得由 agent 自主修改，必须经用户确认（安全边界）
- 复盘分析必须止于「可控变量」，禁止将失误归因于外部条件
- 失误分析用 5 Why 追溯根本原因，而非停在表面现象

### 配置入口

用户可通过以下提示词一次性完成配置：

> "帮我配置每日复盘系统：1. 在 HEARTBEAT.md 添加复盘任务（22:30-23:00 检查）2. 在 AGENTS.md 添加学习规则（每次会话读取 LESSONS.md）3. 创建 LESSONS.md 经验沉淀文件 4. 在 SOUL.md 添加复盘约束（报价需区间+假设，不得自主修改）"
