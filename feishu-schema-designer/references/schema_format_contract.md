...
> verified: 2026-04-16（亲自读完 lark-base workflow-schema / form-questions / dashboard-block / role-config 后重写）

`schema.json` 必须 100% 对齐 lark-base 期望格式。本文件**用 lark-base 原生 key 名**——发明新 key 名会在 Stage 5 导致 lark-cli 命令失败。

## 为什么不复制 lark-base reference

lark-base 的 reference 是单一可信源（SSOT）。**一旦本文件复制了 lark-base 内容，就有版本漂移风险**。本文件**指向 lark-base reference 路径**+ **列出 lark-base 认可的 key 名**，但不复制示例本身。

## 强制查阅的 lark-base reference（Stage 3 前必读）

| 我要做什么 | 必查 reference（必亲自 Read） |
|----------|--------------|
| 字段 | [`../../lark-base/references/lark-base-shortcut-field-properties.md`](../../lark-base/references/lark-base-shortcut-field-properties.md) |
...
## 字段类型：业务层 schema vs lark-cli 调用层（2026-04-16 实测重写）

⚠️ **重大澄清**：本 skill 的 `04_schema_final.json` 是**业务层抽象**，便于设计推理；lark-cli `+field-create --json` 用的是**调用层 schema**（扁平 + `style` 子对象）。Stage 5 执行前必须 transform。
...
| `single_link` | `link` | `link_table: <表名>, bidirectional: false` | |
| `duplex_link` | `link` | `link_table: <表名>, bidirectional: true, bidirectional_link_field_name: <反向名>` | |
...
### Transformer 模板（业务层 → lark-cli 调用层）
...
**Step type 枚举**（原文）：
- Trigger: `AddRecordTrigger` / `SetRecordTrigger` / `ChangeRecordTrigger` / `TimerTrigger` / `ReminderTrigger` / `LarkMessageTrigger`
- Action: `AddRecordAction` / `UpdateRecordAction` / `DeleteRecordAction` / `FindRecordAction` / `LarkMessageAction` / `AIClassifyAction` / `AIAgentAction` / 等（查 workflow-schema §Action）
- Branch: `IfElseBranch` / `SwitchBranch` / `AIClassificationBranch`
- System: `Loop` / `End`
...
`data_config.series[].rollup` 枚举：`SUM` / `MAX` / `MIN` / `AVERAGE`

`group_by[].mode`：`integrated`（聚合）/ `enumerated`（多值拆分）

`filter.conjunction`：`and` / `or`

`filter.conditions[].operator`（按字段类型不同）：
- 文本/电话/URL：`is / isNot / contains / doesNotContain / isEmpty / isNotEmpty`
- 数字：`is / isNot / isGreater / isGreaterEqual / isLess / isLessEqual / isEmpty / isNotEmpty`
- 单选：`is / isNot / isEmpty / isNotEmpty`
- 日期：`is / isGreater / isGreaterEqual / isLess / isLessEqual / isEmpty / isNotEmpty`
...
## Role 结构（强制 key 名）— AdvPermBaseRoleConfig

顶层字段：**`role_name` / `role_type` / `base_rule_map` / `table_rule_map` / `dashboard_rule_map` / `docx_rule_map`**。
...
**禁止**用 `permissions` 数组结构——那不是 lark-base 认的格式。
...
5. **禁止发明 JSON key**——不要自造 `widgets / actions / fields / permissions / panels / pages` 等 lark-base 不认识的字段
